...
static const string kCudaLaunchSymName = "cudaLaunchKernel";
...
bool hasExceededProfilingLimit(
  ...
  if (steady_clock::now() - startTime >= duration) {
...
int main(int argc, char* argv[]) {
  ...
  auto duration = seconds(env.duration_sec);

  time_t ttp =
      system_clock::to_time_t(system_clock::now());
  fmt::print("Started profiling at {}", ctime(&ttp));
  ...
  ttp = system_clock::to_time_t(system_clock::now());
  fmt::print("Stopped profiling at {}", ctime(&ttp));
